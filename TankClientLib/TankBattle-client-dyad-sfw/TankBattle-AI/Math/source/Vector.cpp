...
float Vec2::getAngle(bool zero360, bool radians)
{
	if (radians)
		return atan2f(y, x);
	if (zero360)
	{
		float ans = atan2f(y, x) * RAD2DEG;
		if (ans < 0)
			ans += 360.0f;
		return ans;
	}
...
float Vec2::getAngleBetween( Vec2 &v2, bool zero360, bool radians)
{
	float ans;
	float ang1 = getAngle(true);
	float ang2 = v2.getAngle(true);
...
float Vec2::getSlopeBetween(Vec2 v2, bool yInt)
{
	float result = ((v2.y - y) / (v2.x - x));
	if (yInt)
	{
		//Y = MX + b
		//0 = MX + b - Y
